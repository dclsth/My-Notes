...
SET ---> {}   ---> kayak array tapi acak dan cuma bisa add sama remove aja. ga boleh ada duplicates

TUPLE  -->  () --> ga bisa diubah cuma bisa diitung isi dari array. boleh duplikat

DICTIONARY

isi = {"Indonesia" : "Jakarta",
        "China" : "Beijing"}

print (isi.get("Indonesia"))
isi.update({"Germany" : "Berlin"})
# isi.pop("China")
# isi.popitem()
# isi.clear()
...
RANDOM
 ...
 number = random.random() ----> 0.0 - 1.0
 number = random.randint (1,6)  ---> 1 - 6
 mychoice = random.choice(option) ----> pick one dari beberapa option
 random.shuffle(cards)  ---> shuffle cards


 FUNCTION AND DEFAULT

 def transaction (price, tax=0,1):    ----> yang ngga dikasih defaultr harus didahulukan, dan nanti kalo default mau diganti bisa
    return price - (price * tax)

*ARGS AND **KWARGS
ARGS --->  menggabungkan menjadi tuple
